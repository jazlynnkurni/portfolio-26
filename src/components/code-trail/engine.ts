...
// Chase rate ramps DOWN the chain — the head snaps toward the pointer while
// the tail lags, which is what makes the ribbon read as dragged mass rather
// than a rigid object. slot 0 (head) ≈ 0.34, tail ≈ 0.16.
const CHASE_HEAD = 0.34;
const CHASE_TAIL = 0.16;

export function chaseRate(slot = 0, total = ROWS): number {
  const t = total <= 1 ? 0 : Math.min(1, Math.max(0, slot / (total - 1)));
  return CHASE_HEAD + (CHASE_TAIL - CHASE_HEAD) * t;
}

export function badgeCount(index: number, total: number): number {
  // Spindle by depth: 1 at the head, swelling to MAX through the belly, back
  // to 1 at the tail — gives the ribbon a shape instead of a sheared rectangle.
  const t = total <= 1 ? 0 : index / (total - 1);
  const swell = Math.sin(t * Math.PI); // 0 → 1 → 0
  return 1 + Math.round(swell * (MAX_BADGES - 1));
}
