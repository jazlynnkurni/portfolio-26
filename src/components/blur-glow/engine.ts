...
export class BlurGlow {
  ...
  private resolveFont() {
    ...
    try {
      ...
      probe.style.cssText =
        "position:absolute;visibility:hidden;font-family:" + FONT;
  ...
  private buildMask() {
    const gl = this.gl;
    if (!gl) return;
    const m = makeWordMask(
      WORDS[this.wordIdx % WORDS.length],
      this.canvas.width,
      this.canvas.height,
      this.fontFamily,
    );
  ...
  private resize() {
    ...
    if (this.canvas.width === w && this.canvas.height === h && this.levels.length)
      return;
    this.canvas.width = w;
  ...
  private beginMorph() {
    ...
    this.paletteTo = paletteUniforms(
      PALETTES[(this.paletteIdx + 1) % PALETTES.length],
    );
  }
  private finishMorph() {
  ...
  private render(bloom: number, phase: number) {
    ...
    if (this.morphing) {
      this.uploadPalette(
        lerpPaletteUniforms(this.paletteFrom, this.paletteTo, this.morphEased),
      );
    }
