...
/**
 * FELT-LAB — tune the inner playing surface.
 * -------------------------------------------------------------------
 * Locked Burl Maple rail + chrome balls + pockets. The inner surface can toggle
 * between Woven Felt and Velvet, and its colour (the turquoise) is a live
 * picker. Sheen slider tunes the surface highlight. Physics kept. Not in nav.
 */
...
const PAL = { envTop: "#E0752E", envBottom: "#3d2410", rim: "#E9B84A" };
// locked Burl Maple rail
const WOOD = { light: "#B2743C", dark: "#5C3618", ring: 9, warp: 1.35, gloss: 0.5 };

const VERT = `attribute vec2 p; void main(){ gl_Position=vec4(p,0.,1.); }`;
const FRAG = `
precision highp float;
uniform vec2 uRes; uniform float uTime, uEnergy, uSheen;
uniform int uCount, uSurface;
...
vec3 wood(vec2 uv, vec2 C, vec2 outerH, float bevel){
  ...
  float fiber = fbm(gp*vec2(2.0,40.0)); float pores = smoothstep(0.72,0.8, hash(floor(gp*vec2(90.0,30.0))));
  vec3 c = mix(uWoodDark, uWoodLight, 0.35+0.5*grain+0.3*fiber);
  c *= 1.0-0.18*pores; c *= 0.7+0.55*bevel; c += uGloss*pow(bevel,10.0)*0.5; return c;
}
vec3 surface(vec2 uv){
  if(uSurface==1){ // velvet
    float nap=fbm(vec2(uv.x*240.0, uv.y*22.0));
    float streak=0.5+0.5*sin(uv.y*24.0);
    vec3 c=uFelt*(0.66+0.52*nap);
    c+=uSheen*0.14*pow(streak,2.0)*vec3(1.0,0.98,0.9);
    return c;
  } else if(uSurface==2){ // flat / blank — solid colour, matte, no pattern
    return uFelt*(0.92+0.05*fbm(uv*30.0));
  } else { // woven felt
    ...
    return uFelt*(0.80+0.28*thread)+fuzz*0.06+uSheen*0.05*pow(thread,3.0);
  }
}
void main(){
  ...
  else if(dInner>0.0){ float railT=clamp(dInner/0.078,0.0,1.0); float bevel=sin(railT*PI); col=wood(uv,C,outerH,bevel); if(dInner<0.008) col*=0.35; col*=smoothstep(-0.004,0.03,pk); }
  else { vec3 c=surface(uv); float cushShadow=smoothstep(0.0,-0.09,dInner); c*=1.0-0.30*cushShadow; c*=0.6+0.4*smoothstep(0.0,0.02,pk); col=c; }
...
export default function FeltLab() {
  const [surface, setSurface] = useState(2); // 0 woven, 1 velvet, 2 flat
  const [felt, setFelt] = useState("#73A090");
  const [lamp, setLamp] = useState("#A6291F");
  const [sheen, setSheen] = useState(0.6);
  const sRef = useRef(surface); sRef.current = surface;
  const feltRef = useRef(felt); feltRef.current = felt;
  ...
  useEffect(() => {
    ...
    const u = { res: U("uRes"), time: U("uTime"), energy: U("uEnergy"), sheen: U("uSheen"), surface: U("uSurface"), count: U("uCount"), pos: U("uBallPos"), col: U("uBallCol"), bscale: U("uBallScale"), stripe: U("uBallStripe"), r: U("uR"), pocketR: U("uPocketR"), ring: U("uRing"), warp: U("uWarp"), gloss: U("uGloss"), pointer: U("uPointer"), pockets: U("uPockets"), envTop: U("uEnvTop"), envBottom: U("uEnvBottom"), rim: U("uRim"), felt: U("uFelt"), woodLight: U("uWoodLight"), woodDark: U("uWoodDark") };
    ...
    gl.uniform3fv(u.woodLight, hexToRgb(WOOD.light)); gl.uniform3fv(u.woodDark, hexToRgb(WOOD.dark)); gl.uniform1f(u.ring, WOOD.ring); gl.uniform1f(u.warp, WOOD.warp); gl.uniform1f(u.gloss, WOOD.gloss);
    ...
    const render = (now: number) => {
      ...
      gl.uniform2f(u.res, canvas.width, canvas.height); gl.uniform1f(u.time, (now - t0) / 1000); gl.uniform1f(u.energy, energyRef.current); gl.uniform1f(u.sheen, sheenRef.current); gl.uniform1i(u.surface, sRef.current);
      gl.uniform3fv(u.felt, hexToRgb(feltRef.current));
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center py-10" style={{ background: "#FFF5EF" }}>
      ...
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[60] flex flex-col items-center gap-3">
        <div className="flex items-center gap-4 px-4 py-2.5 rounded-2xl" style={{ background: "rgba(30,24,18,0.92)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.12)" }}>
          {/* surface toggle */}
          <div className="flex items-center gap-1">
            {["Woven Felt", "Velvet", "Flat"].map((label, i) => (
              <button key={label} onClick={() => setSurface(i)} className="px-3 py-1.5 rounded-full text-[12.5px] font-mono transition-all" style={{ backgroundColor: surface === i ? "#C97836" : "transparent", color: surface === i ? "#fff" : "rgba(255,255,255,0.7)" }}>{label}</button>
            ))}
          </div>
          <span style={{ width: 1, height: 22, background: "rgba(255,255,255,0.14)" }} />
          {/* felt color */}
          <label className="flex items-center gap-2 text-[11px] font-mono cursor-pointer" style={{ color: "rgba(255,255,255,0.7)" }}>
            felt <input type="color" value={felt} onChange={(e) => setFelt(e.target.value)} className="w-7 h-7 rounded-md bg-transparent cursor-pointer" style={{ border: "1px solid rgba(255,255,255,0.15)" }} />
          </label>
          {/* presets */}
          <div className="flex items-center gap-1.5">
            {PRESETS.map((c) => (
              <button key={c} onClick={() => setFelt(c)} aria-label={c} className="w-5 h-5 rounded-full" style={{ background: c, border: felt.toLowerCase() === c.toLowerCase() ? "2px solid #fff" : "1px solid rgba(255,255,255,0.2)" }} />
            ))}
          </div>
          <span style={{ width: 1, height: 22, background: "rgba(255,255,255,0.14)" }} />
          {/* lamp color */}
          <label className="flex items-center gap-2 text-[11px] font-mono cursor-pointer" style={{ color: "rgba(255,255,255,0.7)" }}>
            lamp <input type="color" value={lamp} onChange={(e) => setLamp(e.target.value)} className="w-7 h-7 rounded-md bg-transparent cursor-pointer" style={{ border: "1px solid rgba(255,255,255,0.15)" }} />
          </label>
          <span style={{ width: 1, height: 22, background: "rgba(255,255,255,0.14)" }} />
          <label className="flex items-center gap-2 text-[11px] font-mono" style={{ color: "rgba(255,255,255,0.7)" }}>sheen <input type="range" min={0} max={1.5} step={0.02} value={sheen} onChange={(e) => setSheen(parseFloat(e.target.value))} className="accent-[#E0965A] w-20" /></label>
          <button onClick={reset} className="px-3 py-1.5 rounded-full text-[12px] font-mono" style={{ color: "rgba(255,255,255,0.6)", border: "1px solid rgba(255,255,255,0.14)" }}>reset ↺</button>
        </div>
        <p className="font-mono text-[11px]" style={{ color: "rgba(255,255,255,0.45)" }}>{surface === 1 ? "Velvet" : surface === 2 ? "Flat / blank" : "Woven felt"} surface · Burl Maple rail (locked) · drag to break</p>
      </div>
    </main>
  );
}
